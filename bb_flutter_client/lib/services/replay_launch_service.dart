...
class ReplayLaunchService {
  ...
  static String buildPersistentSteamLaunchOptions(String existing) {
    ...
    // Remove old +exec biobase_replay if present (replaced by +exec autoexec).
    updated = updated.replaceAll(
      RegExp(r'(^|\s)\+exec\s+biobase_replay(?:\.cfg)?(?=\s|$)', caseSensitive: false),
      ' ',
    );
    ...
    // Use +exec autoexec — CS2 runs autoexec.cfg more reliably than a custom cfg.
    if (!RegExp(
      r'(^|\s)\+exec\s+autoexec(?:\.cfg)?(?=\s|$)',
      caseSensitive: false,
    ).hasMatch(updated)) {
      updated = '$updated +exec autoexec'.trim();
    }
